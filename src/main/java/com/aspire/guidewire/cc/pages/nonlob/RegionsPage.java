...
public class RegionsPage extends LoadableComponent<RegionsPage> {
	...
	public WebElement inpAreasCovered_14;
	...
	public WebElement inpAreasCovered_22;
	...
	public RegionsPage(WebDriver driver) throws Exception{
		this.driver = driver;
	...
	protected void isLoaded() {
		if (!isPageLoaded) {
			Assert.fail();
		}if (isPageLoaded && !(BrowserActions.waitForElementToDisplay(driver, inpName_7))) {
	...
	public void verifyAddedRegionsIsDisplayed(WebDriver driver, String region) {
		String pageNo=lblLastPageNo.getText();
		String num=pageNo.replaceAll("[^0-9]","");
		int no=Integer.parseInt(num);
		for(int i=1;i<=no;i++) {
			By regionBy = By.xpath("//div[text()='"+region+"']");
			int size = BrowserActions.getWebElementSize(driver, regionBy, "region by",0.5);
			if (size > 0) {
			By addesRegionsBy =By.xpath("//div[text()='"+region+"']");
			BrowserActions.waitForElementToDisplay(driver, addesRegionsBy, "addes Regions");
			WebElement addesRegions = addesRegionsBy.findElement(driver);
			Log.assertThat(addesRegions.isDisplayed(), "Added regions is Displayed", "Added regions is not displayed");
			break;
			}
			else {
	...
	public void clickRegionsCheckBox(WebDriver driver,String region) {
		String pageNo=lblLastPageNo.getText();
		String num=pageNo.replaceAll("[^0-9]","");
		int no=Integer.parseInt(num);
		for(int i=1;i<=no;i++) {
			BrowserActions.waitForNetworkIdleState(driver,1 );
			By regionBy=By.xpath("//div[text()='"+region+"']//ancestor::td//preceding-sibling::td//descendant::div[contains(@id,'_Checkbox')]");
			int size = BrowserActions.getWebElementSize(driver, regionBy,"regionBy",0.5);
			if (size > 0) {
				By userRegionBy = By.xpath("//div[text()='"+region+"']//ancestor::td//preceding-sibling::td//descendant::div[contains(@id,'_Checkbox')]");
				BrowserActions.waitForElementToDisplay(driver, userRegionBy,"userRegion");
				WebElement userRegion = userRegionBy.findElement(driver);
				BrowserActions.click(driver, userRegion, "Check box");
				break;
			}
			else {
				BrowserActions.click(driver, btnNextArrow,"Next Button");
	...
	public class AddRegionPage extends LoadableComponent<AddRegionPage> {
		...
		protected void isLoaded() {
			if (!isPageLoaded) {
				Assert.fail();
			}if (isPageLoaded && !(BrowserActions.waitForElementToDisplay(driver, inpFieldHeading))) {
				Log.fail("Page did not open up. Site might be down.", driver);
			}elementLayer = new ElementLayer(driver);
		...
		public AddRegionPage enterEdgePolicyholderEnglish(WebDriver driver,String localization) {
			BrowserActions.type(driver, inpEdgePolicyholderEnglish,localization, "Edge Policyholder English");
			return this;
		}
		public AddRegionPage enterEdgeEnglish(WebDriver driver,String localization) {
