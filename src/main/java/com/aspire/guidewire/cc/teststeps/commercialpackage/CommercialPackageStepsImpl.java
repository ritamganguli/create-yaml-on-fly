...
public class CommercialPackageStepsImpl implements CommercialPackageSteps {
	...
	public void addCoveragesByType(ChooseCoveragesByType chooseCovearges) {
		...
		switch(chooseCovearges.getByCoverageTypeName()) {

		case BY_COVERAGE_TYPE_U:
			actionsPage.moveToU(DriverManager.getDriver());
			if(chooseCovearges.getBycoverageType_U().getUnderground_Resources_Cov_Gen_Damages()!=null) {
				actionsPage.clickUndergroundResourcesCovGenDamages(DriverManager.getDriver());
			}
			createNewExposure(chooseCovearges.getBycoverageType_U().getUnderground_Resources_Cov_Gen_Damages());
			break;

		case BY_COVERAGE_TYPE_L:
			actionsPage.moveToL(DriverManager.getDriver());
			if(chooseCovearges.getBycoverageType_L().getContractual_Liab_Personal_Advertising_Injury()!=null) {
				actionsPage.clickLtdContractualLiabForPersonalAdvertisingInjury(DriverManager.getDriver());
			}
			createNewExposure(chooseCovearges.getBycoverageType_L().getContractual_Liab_Personal_Advertising_Injury());
			break;

		case BY_COVERAGE_TYPE_E:
			actionsPage.moveToE(DriverManager.getDriver());
			if(chooseCovearges.getBycoverageType_E().getEmployee_Benefits_Liability()!=null) {
				actionsPage.clickEmployeeBenefitsLiability(DriverManager.getDriver());
			}
			createNewExposure(chooseCovearges.getBycoverageType_E().getEmployee_Benefits_Liability());
			break;
		}
	...
	public void addCoverages(ChooseCoverages chooseCoverages) {
		...
		case LIQUOR_LIABILITY_ENDORSEMENT:
			actionsPage.moveToPolicyLevelCoverage(DriverManager.getDriver());
			if(chooseCoverages.getPolicy_Level_Coverage().getLiquor_Liability_Endorsement().getLiquor_Liability_Cov_BI()!=null) {
				actionsPage.moveToLiquorLiabilityEndorsement(DriverManager.getDriver())
				.clickLiquorLiabilityCov_BI(DriverManager.getDriver());
				createExposureInjuryIncident(chooseCoverages.getPolicy_Level_Coverage().getLiquor_Liability_Endorsement().getLiquor_Liability_Cov_BI());
			}
			break;
		case POLICY_LEVEL_COVERAGES:
			actionsPage.moveToPolicyLevelCoverage(DriverManager.getDriver());
			if((chooseCoverages.getPolicy_Level_Coverage().getContractual_Liability_Railroads().getContract_Liability_RR_BI())!=null) {
				actionsPage.moveToContractualLiabilityRailroads(DriverManager.getDriver())
				.clickContractLiabilityRR_BI(DriverManager.getDriver());
				createExposureInjuryIncident(chooseCoverages.getPolicy_Level_Coverage().getContractual_Liability_Railroads().getContract_Liability_RR_BI());
			}else {
				if(chooseCoverages.getPolicy_Level_Coverage().getLiquor_Liability_Endorsement().getLiquor_Liability_Cov_BI()!=null) {
					actionsPage.moveToLiquorLiabilityEndorsement(DriverManager.getDriver())
					.clickLiquorLiabilityCov_BI(DriverManager.getDriver());
	...
	public void createNewExposure(Coverage Coverage){
		NewExposurePage newExposurePage = new NewExposurePage(DriverManager.getDriver())
				...
				.enterDamageDescription(DriverManager.getDriver(), Coverage.getDamageDescription());
	...
	public void createExposureInjuryIncident(Coverage Coverage){
		...
		new InjuryIncidentPage(DriverManager.getDriver())
		.selectInjuredPerson(DriverManager.getDriver(), Coverage.getInjuredPerson())
	...
	public void addClaimService(ServiceDetails serviceDetails) {
		...
		new AddServicePage(DriverManager.getDriver()).clickSpecialistMenuIcon(DriverManager.getDriver())
		...
		addServicePage.selectServiceAddress(DriverManager.getDriver())
	...
	public void createReserve(Reserve reserve) {
		...
		new ExposuresPage(DriverManager.getDriver()).clickCreateReserve(DriverManager.getDriver());		new EditReservesPage(DriverManager.getDriver()).selectExposure(DriverManager.getDriver())
		...
		.verifyReserveCreated(DriverManager.getDriver(),reserve.getByCoverageType());
	...
	public void newDocumentClaim(ChooseDocumentsByType chooseDocumentsByType) {
		...
		case UPLOAD_DOCUMENT:
			...
			.selectStatus(DriverManager.getDriver(), chooseDocumentsByType.getUploadDocument().getStatus())
			.selectUploadDocumentType(DriverManager.getDriver(), chooseDocumentsByType.getUploadDocument().getDocumentType())
	 ...
	 * Verify the "No Data to Display" Message
	 * Click on the open "Work Plan" Check box
	 * Click on "Complete" button
	 * Verify "No Data to Display" Message
	 * Click on "Action" and Click on "Close Claim"
	 * Enter the "Notes" and select "Outcome"
	 * Click on "Close Claim"
	 * Verify the Claim "Status" has "Closed"
	 * @param closeClaim
	 */
	@Override
	public void closeClaim(CloseClaim closeClaim) {
		...
		.verifyNoDataMessage(DriverManager.getDriver(), closeClaim.getDataMessage());
		Log.messageStep("Verified "+closeClaim.getDataMessage()+" message in Work plan Page",DriverManager.getDriver());	
	
		new ActionsPage(DriverManager.getDriver()).clickActions(DriverManager.getDriver()).get()
				.clickCloseClaim(DriverManager.getDriver());
		new CloseClaimPage(DriverManager.getDriver()).get()
		...
		Log.messageStep("Claim is Closed successfully ",DriverManager.getDriver());
	
	}

	@Override
	public void reOpenClaim(ReOpenClaim reOpenClaim) {
		new ActionsPage(DriverManager.getDriver()).get().clickActions(DriverManager.getDriver()).clickReopenClaim(DriverManager.getDriver());
		new ReopenClaimPage(DriverManager.getDriver()).enterNote(DriverManager.getDriver(),reOpenClaim.getNote())
		.selectReason(DriverManager.getDriver(),reOpenClaim.getReOpenReason())
		.clickReopenClaim(DriverManager.getDriver()).clickSummary(DriverManager.getDriver())
		.clickStatus(DriverManager.getDriver())
		.VerifyStatus(DriverManager.getDriver(), reOpenClaim.getOpenStatus());
		Log.messageStep("Claim Reopened is Processed ",DriverManager.getDriver());
	}
