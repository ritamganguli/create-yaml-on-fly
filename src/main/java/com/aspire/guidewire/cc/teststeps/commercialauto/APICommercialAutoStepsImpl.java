...
public class APICommercialAutoStepsImpl implements CommercialAutoSteps {
	...
	public void SelectInvolvedPolicyVehicle() {
		...
		// check the vehicle involved in the policy
		String csrfToken = CCAPIUtils.getCSRFToken();
		String cookieSessionID = CCAPIUtils.getCookieSessionID();
		String fileUploadURL = CCAPIUtils.getFileUploadURL();
		
		String selectVehiclePayload = CCAPIUtils.readFormDataFile("auto","1.SelectInvolvedPolicyVehicle");
		selectVehiclePayload = selectVehiclePayload.replace("<CSRF_TOKEN>", csrfToken);
		selectVehiclePayload = selectVehiclePayload.replace("<FILE_UPLOAD_URL>", fileUploadURL);

		Response searchVehicleResponse = CCAPIUtils.sendPostRequest(cookieSessionID, csrfToken, selectVehiclePayload);
		Log.message("search Vehicle response: "+searchVehicleResponse);
		CCAPIUtils.setClaimantName(searchVehicleResponse);
	}
	...
	public void addClaimInformation(ClaimInfo claimInfo) {
		...
		Log.message("refreshForState response: "+refreshForStateResponse);
