...
public class CC_MessageQueueSuite {
	...
	public void TC002(String browser) throws Exception {
		...
		try {
			...
			NonLobImpl cc = NonLobSteps.create();
			cc.loginIntoTheApplication("su", "gw");
			cc.resumeOrRestartMessageQueues(MessageQueue.builder().build());
			Log.testCaseResult(driver);
	...
	public void TC003(String browser) throws Exception {
		...
		try {
			...
			NonLobImpl cc = NonLobSteps.create();
			cc.loginIntoTheApplication("su", "gw");
			cc.resumeOrRestartMessageQueues(MessageQueue.builder().restartMessageQueue(true).build());
			Log.testCaseResult(driver);
